...
DIRNAMES = utils.link_ids_to_input(config["dirs"]["data"], config["sample_regex"]["id"])
...
rule run_pipeline:
    input:
        read_alignment(
            expand(
                os.path.join(config["dirs"]["output"], "counts", "{sample}.txt"),
                sample=IDS,
            )
        ),
    output:
...
rule create_count_matrix:
    ...
    output:
        count=os.path.join(config["dirs"]["output"], "matrix", "count_matrix.csv"),
    script:
...
rule run_multiqc:
    ...
    conda:
        "envs/multiqc.yaml"
    shell:
        "multiqc {params.dir} -o {params.loc} -m fastp "
...
rule combine_data:
    input:
        cmat=os.path.join(config["dirs"]["output"], "matrix", "count_matrix.csv"),
...
rule preprocess_data:
    ...
    output:
        cmat=os.path.join(
            config["dirs"]["output"], "matrix", "filtered_count_matrix.csv"
        ),
        tpm=os.path.join(config["dirs"]["output"], "matrix", "filtered_tpm_matrix.csv"),
        meta=os.path.join(config["dirs"]["output"], "metadata", "filtered_metadata.csv"),
    script:
...
rule normalize_data:
    ...
    output:
        cmat=os.path.join(
            config["dirs"]["output"], "final", "normalized_log_matrix.csv"
        ),
        tpm=os.path.join(config["dirs"]["output"], "final", "normalized_tpm_matrix.csv"),
        meta=os.path.join(config["dirs"]["output"], "final", "metadata.csv"),
    script:
...
rule impute_dropouts:
    ...
    params:
        plot_dir=os.path.join(config["dirs"]["output"], "plots"),
    output:
        cmat=os.path.join(config["dirs"]["output"], "final", "imputed_log_matrix.csv"),
        tpm=os.path.join(config["dirs"]["output"], "final", "imputed_tpm_matrix.csv"),
    script:
